...
model = NDTRSModel(num_classes=3, xdim=[batch_size, 2, 250], kadj=2, num_out=250, dropout=0.).to(device) #adj_matrix=nn.Parameter(torch.FloatTensor(63, 63).cuda()) #, xdim=[batch_size, 63, 5], kadj=2, num_out=5, dropout=0.
...
for epoch in range(num_epochs):
    ...
    if (total_test_acc / len(test_dataset)) > min_acc:
        min_acc = total_test_acc / len(test_dataset)
    ...
    print("Epoch: {}/{} ".format(epoch + 1, num_epochs),
          ...
          "Training Accuracy: {:.4f} ".format(total_train_acc / len(train_dataset)),
          "Test Loss: {:.4f} ".format(total_test_loss / len(test_loader)),
          "Test Accuracy: {:.4f}".format(total_test_acc / len(test_dataset))
          )
print(min_acc)
