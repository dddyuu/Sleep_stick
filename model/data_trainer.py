...
def online_train_epoch(model, source_loader, target_loader, optimizer, device, epoch, total_epochs):
    train_loss = 0.0
    total_val_loss = 0.0
    coarse_accuracy_1 = 0.0
    fine_accuracy_1 = 0.0
    coarse_accuracy_2 = 0.0
    fine_accuracy_2 = 0.0
    test_accuracy = 0.0
    all_original_preds = []
    ...
    total_loss = coarse_loss_total = fine_loss_total = domain_loss_total = alignment_loss_total = 0
    total_train_acc = 0
    target_data, _, _, _, _, _ = target_loader
    ...
    source_original_labels) in enumerate(source_loader):
        ...
        # 获取概率而不是硬标签
        source_coarse_probs_1 = F.softmax(source_coarse_output_1, dim=1)
        source_fine_probs_1 = F.softmax(source_fine_output_1, dim=1)
        source_coarse_probs_2 = F.softmax(source_coarse_output_2, dim=1)
        source_fine_probs_2 = F.softmax(source_fine_output_2, dim=1)
        ...
        # 在第516行附近的 domain_loss 计算处，添加值范围限制
        domain_loss = criterion_domain(
            torch.clamp(source_domain_output, min=1e-7, max=1 - 1e-7),
            source_domain
        ) + criterion_domain(
            torch.clamp(target_domain_output, min=1e-7, max=1 - 1e-7),
            target_domain
        )
        alignment_loss_1 = criterion_alignment(source_features, target_features,
                                               [source_coarse_labels_1, source_fine_labels_1],
    ...
    # with torch.no_grad():
    data, coarse_labels_1, fine_labels_1, coarse_labels_2, fine_labels_2, original_labels = target_loader
...
def new_load_model_weights_predict(model_path,source_loader,target_loader):
    ...
    for batch_idx, (target_loader_idx) in enumerate(target_loader):
        min_acc = -1
        model.load_state_dict(torch.load(model_path))
        best_preds = []
        for epoch in range(NUM_EPOCHS):
            train_loss, test_loss, coarse_accuracy_1, fine_accuracy_1, coarse_accuracy_2, fine_accuracy_2, test_accuracy,all_original_preds = online_train_epoch(
                model, source_loader, target_loader_idx, optimizer, DEVICE, epoch, NUM_EPOCHS
            )
